...
class SiglipAttention(nn.Module):
    ...
    def __init__(self, config, idx):
        ...
        print(f"in siglip attention module. embed dim for proj: {self.embed_dim}", file=sys.stderr, flush=True)
        if not idx == 25:
            ...
            quant_ratio = calculate_quant_ratio(config.torch_dtype)
            
            input_size_per_partition = self.embed_dim
            output_partition_sizes = [self.embed_dim]
            ...
            global_quant_ratio = quant_ratio

            self.k_proj = nn.Linear(1,total_size//quant_ratio)
            print(f"k_proj size after reduction by default bnb method: {self.k_proj.weight.shape}", file=sys.stderr, flush=True)
            self.v_proj = nn.Linear(1,total_size//quant_ratio)
            self.q_proj = nn.Linear(1,total_size//quant_ratio)
            self.out_proj = nn.Linear(1,total_size//quant_ratio)
...
class SiglipEncoderLayer(nn.Module):
    def __init__(self, config: SiglipConfig, idx: Optional[int] = None):
        super().__init__()
        self.embed_dim = config.hidden_size
        print(f"config attn implementation: {config._attn_implementation}", file=sys.stderr, flush=True)
        self.self_attn = SIGLIP_ATTENTION_CLASSES[config._attn_implementation](config=config, idx=idx)
...
class SiglipPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            print(f"bias term: {module.k_proj.bias}. bias term shape: {module.k_proj.bias.shape}", file=sys.stderr, flush=True)
            if module.k_proj.bias.shape == torch.Size([self.config.hidden_size**2//global_quant_ratio]):
                module.k_proj.bias = nn.Parameter(torch.zeros(self.config.hidden_size))
            if module.q_proj.bias.shape == torch.Size([self.config.hidden_size**2//global_quant_ratio]):
                module.q_proj.bias = nn.Parameter(torch.zeros(self.config.hidden_size))
            if module.v_proj.bias.shape == torch.Size([self.config.hidden_size**2//global_quant_ratio]):
                module.v_proj.bias = nn.Parameter(torch.zeros(self.config.hidden_size))
            if module.out_proj.bias.shape == torch.Size([self.config.hidden_size**2//global_quant_ratio]):
                module.out_proj.bias = nn.Parameter(torch.zeros(self.config.hidden_size))
